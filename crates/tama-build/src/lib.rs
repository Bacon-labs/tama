...
fn generate_trust_probe(
    ...
    if public_obligations.is_empty() {
        tama_common::write_string(&output_path, "{\n  \"obligations\": []\n}\n")?;
        return Ok(());
    }

    let source_path = probe_dir.join("PrintAxioms.lean");
    let source = trust_probe_source(manifests, &public_obligations);
    tama_common::write_string(&source_path, &source)?;
    let output = run_capture("lake", &["env", "lean", source_path.as_str()], root)?;
    let combined = format!("{}\n{}", output.stdout, output.stderr);
    let report = parse_print_axioms_output(&combined, &public_obligations)?;
    tama_common::write_string(
        &output_path,
...
fn trust_probe_source(manifests: &[ContractManifest], obligations: &[&Obligation]) -> String {
    ...
    modules.dedup();
    let mut out = String::from("-- Generated by Tama. Fallback trust probe using #print axioms.\n");
    ...
    for obligation in obligations {
        out.push_str(&format!(
            "#eval IO.println \"TAMA_AXIOMS_BEGIN {}\"\n",
            obligation.lean_decl
        ));
        out.push_str(&format!("#print axioms {}\n", obligation.lean_decl));
        out.push_str(&format!(
            "#eval IO.println \"TAMA_AXIOMS_END {}\"\n",
            obligation.lean_decl
        ));
    }
    out
