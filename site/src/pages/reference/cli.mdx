...
`NO_COLOR`, `CLICOLOR`, `CLICOLOR_FORCE` — standard color-control variables.
A non-empty `NO_COLOR` or `CLICOLOR=0` disables color; a non-empty
`CLICOLOR_FORCE` enables it even when the destination is not a terminal.
