...
| Real Foundry          | `tama test` runs `forge test` unchanged and mirror tests pass | Tama mutates user Forge filters or replaces Foundry with a fake runner |
...
19. `tama test` is exact passthrough to `forge test`. Wrapper logs go to stderr. Forge stdout/stderr and exit code are preserved.
...
```json
{
  ...
  "coverage": {
    "disposition": "mirror",
    "path": "test/verity/ERC20Lite.t.sol:ERC20LiteTest.testTransferPost"
  }
}
```
...
5. Starter Foundry tests deploy generated bytecode through generated deployer.
...
2. `tama test [forge-args...]` execs `forge test` with args unchanged.
...
- Foundry mirrors:
  - deployment
  - increment success
  - getter mirrors generated bytecode state
- generated bridge:
  - interface
  - deployer
- audit:
